...
exports.encodeID = exports.decodeID = exports.reserveCodeRange = exports.messages = exports.message = exports.assert = void 0;
...
let message = function message(errorCode) {
...
let reserveCodeRange = function reserveCodeRange(first, last, moduleName) {
...
const hashids = new _hashids.default("Art Compiler LLC"); // This string shall never change!
...
const decodeID = id => {
  ...
  id = id.replace(/\+/g, " ");
  let parts = id.split(" ");
  let ids = []; // Concatenate the first two integer ids and the last hash id. Everything
  // else gets erased.
...
const encodeID = ids => {
  ...
  let id = hashids.encode(ids); // console.log("[2] encodeID() << " + id);
...
function postAuth(path, data, resume) {
  ...
  var req = _https.default.request(options);
