...
window.gcexports.viewer = function () {
  ...
  class TableChart extends React.Component {
    ...
    componentDidUpdate() {
      ...
      let height = props.height - 2 * padding || "100%"; // render the table
      ...
      function tabulate(data, columns) {
        ...
        table.attr("width", width + 2 * padding).attr("height", height + 2 * padding); // create a row for each object in the data

        let count = data.length;
        let dy = height / count;
        let textSize = +style.tspan["font-size"] || 12;
        var rows = tbody.selectAll("text").data(data).enter().append("text").attr("x", padding).attr("y", (d, i) => {
          return padding + (i + 1) * dy - (dy - textSize) / 2 - 2;
        });
        var lines = tbody.selectAll("line").data(data.slice(1)).enter().append("line").attr("x1", padding).attr("y1", (d, i) => {
          return padding + (i + 1) * dy;
        }).attr("x2", width + padding).attr("y2", (d, i) => {
          return padding + (i + 1) * dy;
        ...
        }).enter().append("tspan").attr("text-anchor", (d, i) => {
  ...
  class HeatmapChart extends React.Component {
    ...
    componentDidUpdate() {
      ...
      let gridHeight = gridSize / 2; //reset the overall font size

      var newFontSize = width * 62.5 / 900;
      d3.select("html").style("font-size", newFontSize + "%"); // svg container

      d3.select("#chart").html(""); // Clear view.

      var svg = d3.select("#chart").append("svg").attr("width", width + margin.top + margin.bottom).attr("height", h + margin.left + margin.right).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // linear colour scale
      ...
      var timeLabels = svg.selectAll(".timeLabel").data(times).enter().append("text").text(function (d) {
        ...
        return i * gridSize;
      }).attr("y", 0).style("text-anchor", "middle").attr("transform", "translate(" + gridSize / 2 + ", -6)"); // group data by location

      var nest = d3.nest().key(function (d) {
        return d.location;
      }).entries(dataset); // array of locations in the data

      var locations = nest.map(function (d) {
        return d.key;
      });
      var currentLocationIndex = 0; // create location dropdown menu
      ...
      var updateHeatmap = function updateHeatmap(location) {
        console.log("currentLocationIndex: " + currentLocationIndex); // filter data to return object of location of interest
      ...
      locationMenu.on("change", function () {
        ...
        currentLocationIndex = +selectedLocation; // run update function with selected location
...
},{"./cjs/react.development.js":6,"./cjs/react.production.min.js":7,"_process":3}]},{},[1])(1)
});
